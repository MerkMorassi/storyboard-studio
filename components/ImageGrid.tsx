...
const AssignAgentControl: React.FC<{ 
    ...
    return (
        <div className="relative" ref={dropdownRef} onClick={(e) => e.stopPropagation()}>
            ...
            {isOpen && (
                <div className="absolute bottom-full right-0 mb-1 w-48 bg-neutral-800 border border-neutral-700 shadow-lg z-20">
...
export const ImageGrid: React.FC<ImageGridProps> = ({ images, isLoading, error, onViewImage, gridOverlay, onGridOverlayChange, onEditImage, onAddToStoryboard, onAddToInspiration, onUpscaleImage, agents, onAssignAgentToImage, onCreateAgent, agentFilter, onAgentFilterChange, awaitingExternalGeneration }) => {
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-full min-h-[50vh] bg-neutral-900/50 p-8">
        <LoadingSpinner />
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center h-full min-h-[50vh] bg-red-900/20 border border-red-500 p-8 text-center">
        <div>
  ...
  if (awaitingExternalGeneration) {
    return (
        <div className="flex flex-col items-center justify-center h-full min-h-[50vh] bg-neutral-900/50 border-2 border-dashed border-neutral-800 p-8 text-center">
            ...
            <p className="mt-1 text-neutral-500">Your images are being generated by your external service (Tasklet/Perchance). Check your configured output folder for results.</p>
        </div>
    );
  }
    ...
    <div>
        <div className="mb-4 flex flex-col sm:flex-row items-center justify-between gap-4">
             <div className="relative w-full sm:w-auto flex-grow max-w-xs">
                <input
                    ...
                    className="w-full bg-neutral-900 border border-neutral-700 p-2 pl-8 focus:ring-2 focus:ring-neutral-500 focus:border-neutral-500 transition duration-200 outline-none"
                />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-4">
            {images.map((image, index) => (
                <div 
                    key={image.id} 
                    className="relative bg-neutral-900 overflow-hidden group transition-all duration-300 hover:scale-105 shadow-lg cursor-pointer ring-2 ring-transparent hover:ring-neutral-500/50"
                ...
                >
                    ...
                    {image.agentId && (
                        <div className="absolute bottom-2 left-2 p-1 bg-black/60 text-white text-xs font-bold pointer-events-none">
        ...
        {images.length === 0 && agentFilter && (
             <div className="mt-8 flex flex-col items-center justify-center h-full min-h-[30vh] text-center">
