...
import { MagicIcon, ChevronDownIcon, ImageIcon, ListIcon, LoadingSpinner } from './icons.tsx';
...
const AccordionSection: React.FC<{ title: string; sectionId: string; isOpen: boolean; onToggle: () => void; children: React.ReactNode }> = ({ title, isOpen, onToggle, children }) => (
    <div className="border border-accent rounded-lg overflow-hidden bg-secondary/30">
        <button type="button" className="w-full p-3 flex justify-between items-center bg-secondary/50 text-left font-bold text-neutral-200 hover:bg-accent/50 transition-colors" onClick={onToggle}>
            <span>{title}</span>
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto w-full h-full flex flex-col space-y-6 overflow-y-auto">
            ...
            <div className="flex-shrink-0">
                <h2 className="text-3xl font-bold text-neutral-200 mb-2">Image Studio</h2>
                <p className="text-neutral-400">Generate high-fidelity concept art, storyboards, and assets using the MythOS Cinematic Engine.</p>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* Controls Column */}
                <div className="lg:col-span-1 bg-surface p-6 border border-accent rounded-xl space-y-4 h-fit">
                    <div className="flex items-center gap-2 mb-2">
                        <span className="px-2 py-1 rounded bg-purple-900/50 border border-purple-500/30 text-purple-200 text-[10px] font-bold uppercase tracking-wider">Engine: SDXL</span>
                        <span className="px-2 py-1 rounded bg-secondary border border-accent text-neutral-400 text-[10px] font-bold">{modelVersion}</span>
                    </div>
                    ...
                    <AccordionSection title="1. SCENE BUILDER" sectionId="scene" isOpen={openSections.has('scene')} onToggle={() => toggleSection('scene')}>
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            <FormField label="Time">
                                <select value={timeOfDay} onChange={(e) => handleTimeChange(e.target.value as any)} className="w-full bg-secondary border border-accent rounded p-2 text-sm focus:ring-1 focus:ring-brand outline-none">
                        ...
                        <div className="grid grid-cols-12 gap-2">
                            <div className="col-span-6">
                                <FormField label="Location">
                                    <input 
                                        ...
                                        className="w-full bg-secondary border border-accent rounded p-2 text-sm focus:ring-1 focus:ring-brand outline-none" 
                                    />
                            ...
                            <div className="col-span-3">
                                <FormField label="Weather">
                                    <select value={weather} onChange={(e) => handleWeatherChange(e.target.value)} className="w-full bg-secondary border border-accent rounded p-2 text-sm focus:ring-1 focus:ring-brand outline-none">
                            ...
                            <div className="col-span-3">
                                <FormField label="Lighting">
                                    <select value={lighting} onChange={(e) => handleLightingChange(e.target.value)} className="w-full bg-secondary border border-accent rounded p-2 text-sm focus:ring-1 focus:ring-brand outline-none">
                        ...
                        <div className="grid grid-cols-12 gap-2">
                            <div className="col-span-3">
                                <FormField label="Cast Size">
                                    <select value={numCharacters} onChange={(e) => handleNumCharactersChange(parseInt(e.target.value))} className="w-full bg-secondary border border-accent rounded p-2 text-sm focus:ring-1 focus:ring-brand outline-none">
                            ...
                            <div className="col-span-9">
                                <div className="grid grid-cols-3 gap-1">
                                    ...
                                    {numCharacters === 0 && (
                                        <div className="col-span-3 flex items-center justify-center h-full text-neutral-600 text-xs italic border border-accent rounded bg-secondary/50 mt-5 py-2">
                        ...
                        <FormField label="Visual Style">
                            <select value={visualStyle} onChange={(e) => handleStyleChange(e.target.value)} className="w-full bg-secondary border border-accent rounded p-2 text-sm focus:ring-1 focus:ring-brand outline-none">
                                {IMAGE_STYLES.map(s => <option key={s} value={s}>{s}</option>)}
                            </select>
                        </FormField>
                        <div className="grid grid-cols-2 gap-3">
                            <FormField label="Framing">
                                <select value={cameraAngle} onChange={(e) => handleFramingChange(e.target.value)} className="w-full bg-secondary border border-accent rounded p-2 text-sm focus:ring-1 focus:ring-brand outline-none">
                                    {['Extreme Close Up', 'Close Up', 'Medium', 'Wide', 'Extreme Wide'].map(a => <option key={a} value={a}>{a}</option>)}
                                </select>
                            </FormField>
                            <FormField label="Shot Type">
                                <select value={shotType} onChange={(e) => handleShotTypeChange(e.target.value)} className="w-full bg-secondary border border-accent rounded p-2 text-sm focus:ring-1 focus:ring-brand outline-none">
                    ...
                    <AccordionSection title="2. ACTION & STYLE" sectionId="prompt" isOpen={openSections.has('prompt')} onToggle={() => toggleSection('prompt')}>
                        <FormField label="Action / Description">
                            <textarea 
                                ...
                                className="w-full h-32 bg-secondary border border-accent rounded p-2 text-sm focus:ring-1 focus:ring-brand outline-none resize-y" 
                            />
                        </FormField>
                        <div className="flex gap-2">
                            <select onChange={(e) => handleInsertPositiveFromTemplate(e.target.value)} value="" className="flex-1 bg-secondary border border-accent rounded p-1.5 text-xs text-neutral-300 outline-none">
                                <option value="" disabled>+ Add Positive from Template...</option>
                                {promptTemplates.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
                            </select>
                            <select onChange={(e) => handleInsertDynamic(e.target.value)} value="" className="flex-1 bg-secondary border border-accent rounded p-1.5 text-xs text-neutral-300 outline-none">
                    ...
                    <AccordionSection title="3. SETTINGS" sectionId="settings" isOpen={openSections.has('settings')} onToggle={() => toggleSection('settings')}>
                        ...
                        <div className="flex gap-2 mb-4">
                            ...
                            <select onChange={(e) => handleInsertDynamicNegative(e.target.value)} value="" className="flex-1 bg-secondary border border-accent rounded p-1.5 text-xs text-neutral-300 outline-none">
                        ...
                        <FormField label="Aspect Ratio">
                            <div className="grid grid-cols-4 gap-1">
                                {['16:9', '9:16', '1:1', '2.39:1'].map(r => (
                                    <button 
                                        ...
                                        className={`px-1 py-1.5 rounded text-[10px] font-bold border transition-colors ${aspectRatioLabel === r ? 'bg-brand border-brand text-white' : 'bg-secondary border-accent text-neutral-400 hover:text-neutral-200'}`}
                                    >
                                        {r}
                                    </button>
                                ))}
                            ...
                            <div className="flex gap-2 mt-2">
                                <div className="flex-1">
                                    ...
                                    <input type="number" value={width} onChange={e => setWidth(parseInt(e.target.value))} className="w-full bg-secondary border border-accent rounded p-1 text-xs" />
                                </div>
                                <div className="flex-1">
                                    <label className="text-[10px] text-neutral-500">H</label>
                                    <input type="number" value={height} onChange={e => setHeight(parseInt(e.target.value))} className="w-full bg-secondary border border-accent rounded p-1 text-xs" />
                                </div>
                            </div>
                        </FormField>
                        <div className="grid grid-cols-2 gap-4">
                            <FormField label={`Steps: ${steps}`}>
                                <input type="range" min="10" max="50" value={steps} onChange={e => setSteps(parseInt(e.target.value))} className="w-full accent-brand" />
                            </FormField>
                            <FormField label={`Guidance: ${guidanceScale}`}>
                                <input type="range" min="1" max="15" step="0.5" value={guidanceScale} onChange={e => setGuidanceScale(parseFloat(e.target.value))} className="w-full accent-brand" />
                            </FormField>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <FormField label="Seed">
                                <div className="flex gap-2">
                                    <input 
                                        ...
                                        placeholder="Random"
                                        className="flex-grow bg-secondary border border-accent rounded p-2 text-sm outline-none"
                                        disabled={randomizeSeed}
                                    />
                                    <div className="flex items-center gap-1">
                                        <input type="checkbox" checked={randomizeSeed} onChange={e => setRandomizeSeed(e.target.checked)} id="rndSeed" className="accent-brand"/>
                        ...
                        <div className="grid grid-cols-2 gap-4 pt-2 border-t border-accent opacity-50 pointer-events-none">
                            <div className="col-span-1">
                                <label className="flex items-center gap-2 cursor-pointer mb-1">
                                    <input type="checkbox" checked={useUpscaler} onChange={e => setUseUpscaler(e.target.checked)} className="rounded border-neutral-600 bg-secondary text-brand" />
                                ...
                                {useUpscaler && (
                                    <div className="flex items-center gap-1">
                                        ...
                                        <input type="number" step="0.1" min="1" max="2" value={upscaleBy} onChange={e => setUpscaleBy(parseFloat(e.target.value))} className="w-12 bg-secondary border border-accent rounded p-0.5 text-xs text-center" />
                                    </div>
                                )}
                            </div>
                            <label className="flex items-center gap-2 cursor-pointer h-fit">
                                <input type="checkbox" checked={addQualityTags} onChange={e => setAddQualityTags(e.target.checked)} className="rounded border-neutral-600 bg-secondary text-brand" />
                    ...
                    <div className="mt-4 bg-primary border border-accent rounded-lg p-3 relative group">
                        <div className="absolute top-0 right-0 px-2 py-1 bg-secondary text-[10px] text-neutral-400 rounded-bl-lg font-bold tracking-wider">
                            LIVE SCRIPT PREVIEW
                        </div>
                    ...
                    <button
                        ...
                        disabled={isLoading}
                        className="w-full py-3 bg-brand hover:bg-brand-hover text-white font-bold rounded-xl transition-all shadow-lg hover:shadow-blue-500/25 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 mt-2"
                    >
                        {isLoading ? (
                ...
                <div className="lg:col-span-2 space-y-4">
                    <div className="bg-secondary/30 border border-accent rounded-lg flex flex-col relative overflow-hidden min-h-[500px]">
                        <div className="flex-grow flex items-center justify-center bg-primary relative p-4">
                            {isLoading ? (
                    ...
                    {/* Metadata Section */}
                    {metadata && (
                        <div className="bg-secondary/30 border border-accent rounded-lg p-4">
                            <div className="flex items-center gap-2 mb-2 text-neutral-400">
                                <ListIcon className="w-4 h-4" />
                                <span className="text-xs font-bold uppercase tracking-wider">Generation Metadata</span>
                            </div>
                            <div className="bg-primary/50 p-3 rounded-lg border border-accent overflow-x-auto">
